...
fn click_me_btn(
    ...
    mut query: Query<
        (
            &mut UIWidgetState,
            &CssID,
            &mut bevy_extended_ui::widgets::Button,
        ),
        With<CssID>,
    >,
) {
    for (mut state, id, mut button) in query.iter_mut() {
        if id.0.eq("fn_q_key") {
            ...
            if state.disabled {
                button.text = "Im Disabled".to_string();
            } else {
                button.text = "Im Enabled".to_string();
            }
        }
    }
}
...
fn enter_me_btn(
    ...
    mut query: Query<(Entity, &mut bevy_extended_ui::widgets::Button), With<CssID>>,
) {
