...
use regex::Regex;
use crate::io::{CssStylesheet, UiLoadedHandles};
use crate::styles::css_colors::Colored;
use crate::styles::Style;
use crate::styles::style_types::{Background, FontVal, Radius};
...
#[reflect(Resource)]
pub struct UiParsedStyles {
    pub by_ui: HashMap<String, ParsedCss>,
}

fn load_css_for_ui(
    ui_id: &str,
    assets_css: &Assets<CssStylesheet>,
    loaded: &UiLoadedHandles,
) -> ParsedCss {
    let mut merged_vars: HashMap<String, String> = HashMap::new();
    let mut merged_styles: HashMap<String, Style> = HashMap::new();

    let Some(entry) = loaded.by_id.get(ui_id) else {
        return ParsedCss::default();
    };

    for handle in &entry.css {
        if let Some(sheet) = assets_css.get(handle) {
            let (vars, styles) = parse_css_text(&sheet.0);

            for (k, v) in vars {
                merged_vars.insert(k, v);
            }

            for (sel, st) in styles {
                merged_styles.insert(sel, st);
            }
        }
    }
...
pub fn parse_css_text(css: &str) -> (HashMap<String, String>, HashMap<String, Style>) {
    let Ok(stylesheet) = StyleSheet::parse(css, ParserOptions::default()) else {
        return (HashMap::new(), HashMap::new());
    };

    let mut css_vars: HashMap<String, String> = HashMap::new();
    let mut style_map: HashMap<String, Style> = HashMap::new();

    for rule in &stylesheet.rules.0 {
        if let CssRule::Style(style_rule) = rule {
            let Ok(selector) = style_rule
                .selectors
            ...
            // Build Style for non-:root rules
            let mut style = Style::default();
            for property in &style_rule.declarations.declarations {
                let prop_id = property.property_id();
                let name = prop_id.name();

                let Ok(mut value) = property.value_to_css_string(PrinterOptions::default()) else {
                    continue;
                };

                // resolve simple var(--name)
                if let Some(var_name) = value.strip_prefix("var(").and_then(|s| s.strip_suffix(')')) {
                    if let Some(v) = css_vars.get(var_name.trim()) {
                        value = v.clone();
                    }
                }
