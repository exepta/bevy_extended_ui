...
fn update_toggle_button_system(
    ...
    for (toggle_button, id, mut state) in toggle_q.iter_mut() {
        state.checked = toggle_button.selected;
        for (bind_to, mut text) in text_q.iter_mut() {
            if bind_to.0 == id.0 {
                text.0 = toggle_button.label.clone();
            }
        }
    }
}
