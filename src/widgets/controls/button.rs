...
use bevy::prelude::*;
use crate::{widgets, ExtendedUiConfiguration};
use crate::styles::css_attributes::{CssClass, CssSource, TagName};
use crate::styles::css_colors::Colored;
use crate::styles::style_types::IconPlace;
use crate::widgets::{Button, WidgetId, WidgetKind};
use crate::widgets::attributes::{BindToID, CurrentWidgetState, UIGenID, UIWidgetState};
use crate::services::image_cache_service::ImageCache;
    ...
    for (entity, id, button, source_opt) in query.iter() {
        ...
        commands.entity(entity).insert((
            ...
            if button.icon_place == IconPlace::Left {
                place_icon(builder, button, &asset_server, &mut image_cache, id.get(), *layer, css_source.clone());
            }
