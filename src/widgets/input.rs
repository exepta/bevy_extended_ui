...
use crate::widgets::{BindToID, InputCap, InputField, InputType, UIGenID, UIWidgetState, WidgetId, WidgetKind};
...
fn calculate_correct_text_container_width(
    query: Query<(
        &InputField,
        &UIGenID,
    ), (With<InputField>, Without<InputContainer>, Or<(Added<InputField>, Changed<UiStyle>, Changed<InputField>)>)>,
    mut container_query: Query<(&mut UiStyle, &mut OriginalWidth, &BindToID), (With<InputContainer>, Without<InputField>)>,
) {
...
/// - Updates than e visible text and cursor position accordingly.
